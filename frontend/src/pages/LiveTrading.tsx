...
import { Power, ChevronDown, AlertCircle, Check, X, XCircle, RefreshCw, Brain, TrendingUp, TrendingDown, Plus, ArrowRight } from 'lucide-react'
import type { ForexPair, StrategyDefinition, ChartSignalMarker } from '../types'
...
function LiveTrading({
  ...
  const refreshBrokerData = useCallback(async (reason: string) => {
    ...
    const seededBroker = backendBroker?.id
      ? {
          id: backendBroker.id,
    ...
    await Promise.all([
      fetchPositions(broker.id, fetchVersion),
      fetchBalance(broker.id, fetchVersion),
      fetchTradeHistory(broker.id, fetchVersion),
      fetchOrders(broker.id, fetchVersion),
    ])
  }, [backendBroker?.connected, backendBroker?.environment, backendBroker?.id, backendBroker?.name, fetchActiveBroker, fetchBalance, fetchOrders, fetchPositions, fetchTradeHistory, markFeed])

  // Broker data polling
  useEffect(() => {
    void refreshBrokerData('mount')
    const interval = setInterval(() => void refreshBrokerData('poll'), 5000)
    return () => clearInterval(interval)
  }, [refreshBrokerData])
  ...
  useEffect(() => {
    const fetchSignal = async () => {
      ...
      try {
        const data = await api.fetchSignalBreakdown(selectedPair.symbol, chartTimeframe, 'scalp')
    ...
    void fetchSignal()
    const interval = setInterval(() => void fetchSignal(), 10000)
    return () => clearInterval(interval)
  }, [chartTimeframe, markFeed, selectedPair.symbol])
  ...
  useEffect(() => {
    ...
    void fetchQuote()
    const interval = setInterval(() => void fetchQuote(), 5000)
    return () => clearInterval(interval)
  }, [selectedPair.symbol])
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      // Refresh broker data in background (non-blocking)
      void refreshBrokerData('manual-order')
    } catch (err: any) {
  ...
  const chartOverlay = useMemo(() => {
    ...
    const hasOpenTradeContext = Boolean(activePosition || activeOrder)

    const activeExecution = [...recentExecs]
      .filter((execution) => execution.symbol === selectedPair.symbol && execution.status === 'success' && hasOpenTradeContext)
      .sort((left, right) => getTimestampValue(right.created_at) - getTimestampValue(left.created_at))
      .find((execution) => {
    ...
    const executionDetail = activeExecution?.detail as Record<string, unknown> | undefined
    ...
    const executionEntryMin = toFiniteNumber(executionDetail?.entryMin)
    const executionEntryMax = toFiniteNumber(executionDetail?.entryMax)
    const executionStopLoss = toFiniteNumber(executionDetail?.stopLoss)
    const executionTakeProfit1 = toFiniteNumber(executionDetail?.takeProfit1)
    const executionTakeProfit2 = toFiniteNumber(executionDetail?.takeProfit2)
    const executionTakeProfit3 = toFiniteNumber(executionDetail?.takeProfit3)
    const executionConfidence = toFiniteNumber(executionDetail?.confidence)

    const entryMin = (
      executionEntryMin
      ?? analysisForPair?.entryRange?.min
      ?? fallbackSignal?.entryMin
      ?? orderEntry
      ?? positionEntry
    )
    const entryMax = (
      executionEntryMax
      ?? analysisForPair?.entryRange?.max
      ?? fallbackSignal?.entryMax
      ?? orderEntry
      ?? positionEntry
    )

    const normalizedEntryMin = entryMin ?? entryMax ?? orderEntry ?? positionEntry ?? toFiniteNumber(fallbackSignal?.entry)
    const normalizedEntryMax = entryMax ?? entryMin ?? orderEntry ?? positionEntry ?? toFiniteNumber(fallbackSignal?.entry)
    const entryMid = (
      orderEntry
      ?? positionEntry
      ?? toFiniteNumber(fallbackSignal?.entry)
      ?? (
        normalizedEntryMin != null && normalizedEntryMax != null
          ? (normalizedEntryMin + normalizedEntryMax) / 2
          : null
      )
    )

    const stopLoss = (
      toFiniteNumber(activeOrder?.stop_loss)
      ?? executionStopLoss
      ?? analysisForPair?.stopLoss
      ?? toFiniteNumber(fallbackSignal?.stopLoss)
    )
    const takeProfit1 = (
      toFiniteNumber(activeOrder?.take_profit_1)
      ?? executionTakeProfit1
      ?? analysisForPair?.takeProfit1
      ?? toFiniteNumber(fallbackSignal?.takeProfit1)
    )
    const takeProfit2 = (
      toFiniteNumber(activeOrder?.take_profit_2)
      ?? executionTakeProfit2
      ?? analysisForPair?.takeProfit2
      ?? toFiniteNumber(fallbackSignal?.takeProfit2)
    )
    const takeProfit3 = (
      toFiniteNumber(activeOrder?.take_profit_3)
      ?? executionTakeProfit3
      ?? analysisForPair?.takeProfit3
      ?? toFiniteNumber(fallbackSignal?.takeProfit3)
    )
        ...
        : activeOrder?.side === 'sell'
          ...
          : activePosition?.side === 'long'
            ...
            : activePosition?.side === 'short'
              ? 'SELL'
              : executionDetail?.side === 'buy'
                ? 'BUY'
                : executionDetail?.side === 'sell'
                  ? 'SELL'
                  : analysisForPair?.signal === 'buy'
                    ? 'BUY'
                    : analysisForPair?.signal === 'sell'
                      ? 'SELL'
                      : fallbackSignal?.direction
    ) ?? 'HOLD'

    const hasOverlayLevels = [
      normalizedEntryMin,
      normalizedEntryMax,
      entryMid,
      stopLoss,
    ...
    if (hasOpenTradeContext && hasOverlayLevels) {
      return {
        source: 'active' as const,
        signals: [{
          entry: entryMid ?? normalizedEntryMin ?? normalizedEntryMax ?? 0,
          entryMin: normalizedEntryMin ?? entryMid ?? 0,
          entryMax: normalizedEntryMax ?? entryMid ?? 0,
          stopLoss: stopLoss ?? 0,
          takeProfit1: takeProfit1 ?? 0,
          takeProfit2: takeProfit2 ?? 0,
          takeProfit3: takeProfit3 ?? 0,
          signalId: activeOrder?.order_id ?? fallbackSignal?.signalId,
          direction,
          timestamp: activeOrder?.updated_at || activeOrder?.created_at || activeExecution?.created_at || fallbackSignal?.timestamp || new Date().toISOString(),
          status: fallbackSignal?.status ?? 'VALID',
          confidence: analysisForPair?.confidence ?? executionConfidence ?? fallbackSignal?.confidence ?? 100,
          symbol: selectedPair.symbol,
          expiresAt: fallbackSignal?.expiresAt,
        }],
      }
    }
  ...
  }, [chartSignals, lastAnalysis, orders, positions, recentExecs, selectedPair.symbol])
          ...
          <div className="flex items-center justify-between px-3 py-1 border-b border-[#1c2333] bg-[#0d1117] shrink-0">
            ...
            {chartOverlay.source !== 'none' && (
              <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${
                ...
                {chartOverlay.source === 'active' ? 'Active Trade' : 'Signal'}
              </span>
            )}
          </div>
          ...
          <div className="divide-y divide-[#1c2333]/70">
            {feedState.positions.status === 'error' ? (
              ...
              positions.map((pos) => {
                ...
                const isExpanded = expandedPosition === positionRowId
                ...
                return (
                  <div
                    key={positionRowId}
                    className="px-3 py-2 hover:bg-[#161b26]/60 cursor-pointer transition-colors"
                    onClick={() => setExpandedPosition(isExpanded ? null : positionRowId)}
                  >
          ...
          <div className="divide-y divide-[#1c2333]/70">
            {feedState.orders.status === 'error' ? (
              ...
              orders.map((order) => {
                ...
                return (
                  <div key={order.order_id} className="px-3 py-2 hover:bg-[#161b26]/60 transition-colors">
          ...
          {activeTab === 'history' && (
            <div className="divide-y divide-[#1c2333]/70">
              {feedState.history.status === 'error' ? (
                ...
                <div className="px-3 py-8 text-center text-[11px] text-trading-muted">No closed trades yet</div>
              ) : (
                tradeHistory.map((trade) => {
                  const tradePair = getPairBySymbol(trade.symbol) || selectedPair
                  return (
                    <div key={trade.trade_id} className="px-3 py-2 hover:bg-[#161b26]/60 transition-colors">
