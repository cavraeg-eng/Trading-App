"""Prediction response pipeline and cache seam."""
...
def asset_class_for_symbol(symbol: str) -> PredictionAssetClass:
...
async def request_with_account_context(
...
def trade_style_for_strategy_mode(strategy_mode: PredictionStrategyMode) -> str:
...
def build_prediction_response(
    ...
    analysis_provider: Any = analyze_symbol,
    metadata_provider: Any = get_ohlcv_with_metadata,
) -> PredictionResponse:
    ...
    if metadata_override is None:
        _, metadata = metadata_provider(request.symbol, request.timeframe, trade_style=trade_style)
    else:
    ...
    analysis = (
        analysis_provider(request.symbol, request.timeframe, trade_style=trade_style)
    ...
    response = PredictionResponse(
        ...
        asset_class=request.asset_class if request.asset_class != PredictionAssetClass.UNKNOWN else asset_class_for_symbol(request.symbol),
