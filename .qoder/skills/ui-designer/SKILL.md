---
name: ui-designer
description: Design stunning, production-grade web prototypes with React + Vite + Tailwind CSS + shadcn/ui. Use when creating new websites, landing pages, web apps, UI designs, or when the user mentions UI, design, or vibe coding.
---

# UI Designer

Design stunning, stylish, and flawlessly functional web prototypes on the first attempt.
...
```bash
<path-to-this-skill>/scripts/init-project.sh <project-name>
...
4. Generate images for hero banners, illustrations, backgrounds, feature sections — generate ALL images the design needs NOW
...
- **Event Handler Binding:** Is every interactive element bound to the correct handler?
- **Handler Definition:** Is the bound function explicitly defined within the component?
- **State Update Logic:** Does the handler contain actual state update logic? Empty `() => {}` is invalid
- **State Initialization:** Is all required state correctly initialized?
...
- ALWAYS set up the design system FIRST using the scaffolding script
- ALWAYS ensure that buttons can be clicked to perform actual, functional operations — not just static display
- Generate images for the frontend page. Images are essential — they are what make a design look professional. NEVER leave placeholder blocks or skip image generation
